...
function toRecord(row, index) {
  ...
  if (source === 'scimago') base.scimago = { rank: numberOrNull(pick(row, ['rank'])), sjr: numberOrNull(pick(row, ['sjr', 'sjr 2024', 'sjr 2023'])), hIndex: numberOrNull(pick(row, ['h index', 'h-index', 'hindex'])), quartile: pick(row, ['quartile', 'best quartile', 'sjr best quartile']), country: pick(row, ['country', 'country/territory']), category: pick(row, ['category', 'categories']), areas: pick(row, ['areas']), totalDocs: numberOrNull(pick(row, ['total docs. (2025)', 'total docs'])), totalDocs3Years: numberOrNull(pick(row, ['total docs. (3years)', 'total docs 3years'])), totalRefs: numberOrNull(pick(row, ['total refs'])), totalCitations3Years: numberOrNull(pick(row, ['total citations (3years)', 'total citations'])), citableDocs3Years: numberOrNull(pick(row, ['citable docs. (3years)', 'citable docs'])), citationsPerDoc2Years: numberOrNull(pick(row, ['citations / doc. (2years)', 'citations / doc'])), refsPerDoc: numberOrNull(pick(row, ['ref. / doc.', 'refs / doc'])), femalePercent: numberOrNull(pick(row, ['%female', 'female'])), overton: numberOrNull(pick(row, ['overton'])) };
  if (source === 'core') base.core = {
    rank: pick(row, ['rank', 'core rank', 'ranking', 'tier']),
    year: pick(row, ['year', 'ranking year']) || new Date().getFullYear().toString(),
    field: pick(row, ['field', 'discipline', 'primary for']),
    acronym: pick(row, ['acronym']),
    sourceName: pick(row, ['source name', 'ranking source']),
    sourceUrl: pick(row, ['source url', 'core url', 'profile url']),
    dblp: pick(row, ['dblp', 'dblp url'])
  };
